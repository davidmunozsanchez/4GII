(define (problem problema7)
    ...
    (:goal
        (and
            ...
            (estaEn Soldado1 LOC12)
            ;(< (CostePlan) 58) ;;;;Tarda más de 45 minutos con esta restricción, no se ha garantizado el óptimo

        )
    

    )
    (:metric minimize (CostePlan) ;;;Se le indica que minimice la nueva métrica, aunque no hay garantías de óptimo,
        )
)
