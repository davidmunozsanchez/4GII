...
void _atst::draw(_modo modo, float r, float g, float b, float grosor)
{
  ...
  glRotatef(45, -1, 0, 0);
  if (activo[2] == 1)
    pata.draw(EDGES, r_p, g_p, b_p, grosor_select);
  else
  ...
  if (activo[6] == 1)
    pie.draw(EDGES, r_p, g_p, b_p, grosor_select);
  else
...
void _atst::seleccion()
{
  ...
  tronco.draw(SELECT, color.r, color.g, color.b, grosor_select);
  ...
  glRotatef(giro_sobre_eje_cabeza, 0, 1, 0);
  glRotatef(giro_cabeza_vertical, 1, 0, 0);
  cabeza.draw(SELECT, color.r, color.g, color.b, grosor_select);
  ...
  glRotatef(flexion_piernas, 1, 0, 0);
  glRotatef(45, -1, 0, 0);
  pata.draw(SELECT, color.r, color.g, color.b, grosor_select);
  ...
  glTranslatef(0.5, -2 * sin(45) * 2 - factor_pies + factor_tronco, 0.2);
  glRotatef(apertura_pies, 0, 1, 0);
  pie.draw(SELECT, color.r, color.g, color.b, grosor_select);
