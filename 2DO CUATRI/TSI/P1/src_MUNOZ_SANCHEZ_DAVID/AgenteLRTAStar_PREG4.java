...
public class AgenteLRTAStar_PREG4 extends AbstractPlayer {
	 ...
	 */
	public AgenteLRTAStar_PREG4(StateObservation state, ElapsedCpuTimer timer) {
		...
		//Esta clase es igual que LRTA pero se crea un archivo para guardar el mapeo de h si no existe
		File archivo = new File("./src/tracks/singlePlayer/evaluacion/src_MUNOZ_SANCHEZ_DAVID/mapaH_LRTA.txt");
		try {
...
//		}
		...
		//Y posteriormente se actualiza, si existe, se tomará directamente de disco
		try {
            Scanner scanner = new Scanner(new File("./src/tracks/singlePlayer/evaluacion/src_MUNOZ_SANCHEZ_DAVID/mapaH_LRTA.txt"));
...
//		}
	...
	public ACTIONS act(StateObservation state, ElapsedCpuTimer timer) {
		...
		if (pos_avatar.x == portal.x && pos_avatar.y == portal.y) {
			...
			//Cuando se vaya a terminar se guarda en disco el mapeo definitivo
			try {
				File archivo = new File("./src/tracks/singlePlayer/evaluacion/src_MUNOZ_SANCHEZ_DAVID/mapaH_LRTA.txt");
				archivo.delete();
				archivo.createNewFile();
