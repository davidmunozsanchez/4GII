...
// tipos
typedef enum{CUBO, PIRAMIDE, OBJETO_PLY, PLY_PERFIL, ROTACION, EXTRUSION, CONO, CILINDRO, ESFERA} _tipo_objeto;
...
switch (t_objeto){
	...
	case PIRAMIDE: piramide.draw(modo,1.0,0.0,0.0,5);break;
		...
		case PLY_PERFIL: rotacion_perfil.draw(modo, 1.0,0.6,0.0,5);break;
		...
		case CILINDRO: cilindro.draw(modo, 0.0, 1.0, 0.0, 5); break;
		case ESFERA: esfera.draw(modo, 0.0, 0.0, 1.0, 5); break;
	}
...
switch (toupper(Tecla1)){
	...
	case '4':modo=SOLID_COLORS;break;
		...
		case 'M':t_objeto=PLY_PERFIL; break;
	}
glutPostRedisplay();
}
...
//Obtencion del objeto ply con el perfil
ply_perfil.parametros("perfil");
vector <_vertex3f> perfil_ply;

for (int i = 0; i < ply_perfil.vertices.size(); i++)
{
	perfil_ply.push_back(ply_perfil.vertices[i]);
}
rotacion_perfil.parametros(perfil_ply, 10, 0, 0, 0);
